...
def list_products() -> list[Product]:
    # Fetch and transform the product list using a list comprehension
    return [Product.load(product) for product in dao.list_products()]


def get_product(product_id: int) -> Product:
    # Fetch a single product and transform it using the `load` method
    product_data = dao.get_product(product_id)
    return Product.load(product_data) if product_data else None


def add_product(product: dict):
    # Add product directly through DAO
    dao.add_product(product)


def update_qty(product_id: int, qty: int):
    # Validate quantity and update through DAO
    if qty < 0:
        raise ValueError("Quantity cannot be negative")
    dao.update_qty(product_id, qty)
